...
public class Engine {
    ...
    public long indexTextNoTran(AutoBox auto, final int commitCount, long id, String str, byte type, int priority, boolean isRemove) {
        if (id == -1) {
            // -1 is internal default value as NULL
            return -1;
        }
    ...
    public void insertToBox(Box box, KeyWord kw, boolean isRemove) {
        Binder binder;
        if (kw instanceof KeyWordE) {
            binder = box.d("/E", kw.getKeyWord(), kw.getID(), kw.getPosition(), kw.getType(), kw.getO());
        } else {
            binder = box.d("/N", kw.getKeyWord(), kw.getID(), kw.getPosition(), kw.getType(), kw.getO());
        }
        if (isRemove) {
            binder.delete();
        } else {
    ...
    public Iterable<KeyWord> searchDistinct(final Box box, String str) {
        return searchDistinct(box, str, null, Long.MAX_VALUE, Long.MAX_VALUE);
    }

    // startId -> descending order
    public Iterable<KeyWord> searchDistinct(final Box box, final String str,
            Byte[] type, final long startId, final long length) {
        final Iterator<KeyWord> it = search(box, str, type, startId).iterator();
    ...
    public Iterable<KeyWord> search(final Box box, String str) {
        return search(box, str, null, Long.MAX_VALUE);
    }

    public Iterable<KeyWord> search(final Box box, String str, Byte[] type, long startId) {
        if (startId < 0) {
        ...
        ArrayList<KeyWord> append = new ArrayList<KeyWord>();
        if (type != null) {
	        for (int i=0; i<type.length; i++) {
	        	Byte t = type[i];
	        	if (i == 1) {
			        for (KeyWord kw: map) {
			        	kw.setType(t == null ? 0 : t);
			        }
	        	} else {
	        		ArrayList<KeyWord> temp = sUtil.fromString(-1, cs, false);
	        		for (KeyWord kw: temp) {
	        			kw.setType(t == null ? 0 : t);
			        }
	        		append.addAll(temp);
                ...
                return new EngineIterator<KeyWord>() {
                    ...
                    public boolean hasNext() {
                        ...
                        if (currentMaxId > (maxId.id + 1)) {
                            ...
                            iter = box.select(rclass, ql, kw.getKeyWord(), maxId.id, kw.getType()).iterator();
                        }
